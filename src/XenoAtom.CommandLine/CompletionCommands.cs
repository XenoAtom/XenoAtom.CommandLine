...
public sealed class CompletionCommands : CommandGroup
{
    ...
    private sealed class CompletionScriptCommand : NoLicenseCommand
    {
        ...
        private static string GenerateBash(string commandName, string[] invocationArguments, string completeRequestCommandName)
        {
            ...
            sb.AppendLine(")");
            sb.AppendLine("  local -a req=()");
            sb.AppendLine("  req+=(--command-name \"${COMP_WORDS[0]}\")");
            sb.AppendLine("  req+=(--index \"$COMP_CWORD\")");
            sb.AppendLine("  local w");
            sb.AppendLine("  for w in \"${COMP_WORDS[@]}\"; do");
            sb.AppendLine("    req+=(--token \"$w\")");
            sb.AppendLine("  done");
            sb.Append("  local out=$(");
            sb.Append("\"${cmd[@]}\" ");
            sb.Append(QuotePosixSingle(completeRequestCommandName));
            sb.AppendLine(" \"${req[@]}\" 2>/dev/null)");
        ...
        private static string GenerateZsh(string commandName, string[] invocationArguments, string completeRequestCommandName)
        {
            ...
            sb.AppendLine(")");
            sb.AppendLine("  local -a req");
            sb.AppendLine("  req=(--command-name \"$words[1]\" --index $((CURRENT-1)))");
            sb.AppendLine("  local w");
            sb.AppendLine("  for w in \"${words[@]}\"; do");
            sb.AppendLine("    req+=(--token \"$w\")");
            sb.AppendLine("  done");
        ...
        private static string GenerateFish(string commandName, string[] invocationArguments, string completeRequestCommandName)
        {
            ...
            sb.AppendLine("  set -l tokens (commandline -opc)");
            sb.AppendLine("  set -l prefix (string sub -l $cursor -- $line)");
            sb.AppendLine("  set -l idx (math (count $tokens) - 1)");
            sb.AppendLine("  if test (string length -- $prefix) -gt 0; and test (string match -rq '\\s$' -- $prefix)");
            sb.AppendLine("    set idx (count $tokens)");
            sb.AppendLine("  end");
            sb.Append("  set -l invocation ");
            AppendFishArray(sb, invocationArguments);
            sb.AppendLine();
            sb.AppendLine("  set -l req --command-name \"$cmdName\" --index $idx");
            sb.AppendLine("  for t in $tokens");
            sb.AppendLine("    set -a req --token $t");
            sb.AppendLine("  end");
            sb.AppendLine("  set -l out ($invocation " + QuoteFishDouble(completeRequestCommandName) + " $req 2>/dev/null)");
            sb.AppendLine("  if test $status -ne 0");
            sb.AppendLine("    set out ($invocation " + QuoteFishDouble(completeRequestCommandName) + " --command-name \"$cmdName\" --line \"$line\" --cursor \"$cursor\" 2>/dev/null)");
            sb.AppendLine("  end");
            sb.AppendLine("  for c in $out");
            sb.AppendLine("    echo $c");
            sb.AppendLine("  end");
            sb.AppendLine("end");
            sb.AppendLine();
        ...
        private static string GeneratePowerShell(string commandName, string[] invocationArguments, string completeRequestCommandName)
        {
            ...
            sb.AppendLine("  $tokens = @($commandAst.CommandElements | ForEach-Object { $_.ToString() })");
            sb.AppendLine("  $index = $tokens.Count - 1");
            sb.AppendLine("  $prefix = $line.Substring(0, [Math]::Min($cursor, $line.Length))");
            sb.AppendLine("  if ($prefix.Length -gt 0 -and [char]::IsWhiteSpace($prefix[$prefix.Length-1])) { $index = $tokens.Count }");
            sb.AppendLine("  $req = @('--command-name', $cmdName, '--index', $index)");
            sb.AppendLine("  foreach ($t in $tokens) { $req += @('--token', $t) }");
            sb.Append("  $candidates = ");
            sb.Append(BuildPowerShellInvocation(invocationArguments));
            sb.Append(' ');
            sb.Append(QuotePowerShellSingle(completeRequestCommandName));
            sb.AppendLine(" @req 2>$null");
            sb.AppendLine("  if (-not $?) {");
            sb.Append("    $candidates = ");
            sb.Append(BuildPowerShellInvocation(invocationArguments));
            sb.Append(' ');
            sb.Append(QuotePowerShellSingle(completeRequestCommandName));
